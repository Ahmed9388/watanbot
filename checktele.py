...
@sedthon.on(events.NewMessage(outgoing=True, pattern=r"\.بحث ثلاثي"))
async def _(event):
    if ispay2[0] == "yes":
...
@sekthon.on(events.NewMessage(outgoing=True, pattern=r"\.صيد (.*)"))
async def _(event):
    if ispay2[0] == "yes":
        isclaim.clear()
        isclaim.append("on")
        ...
        @sedthon.on(events.NewMessage(outgoing=True, pattern=r"\.حالة البحث"))
        async def _(event):
            if ispay2[0] == "yes":
                if "on" in isclaim:
    ...
    {str(eee)}''')
                    if "A wait of" in str(eee):
                        ...
                        await sedthon.send_message(event.chat.id,"سأستمر بالبحث !"
            else:
                pass
            trys += 1
